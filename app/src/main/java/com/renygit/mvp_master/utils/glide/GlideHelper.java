package com.renygit.mvp_master.utils.glide;
...
public class GlideHelper {
    ...
    public static void disPlayRound(@NonNull ImageView view, Object model, int radiusDp, @DrawableRes int... imgIds){
        try {
            ...
            GlideApp.with(view).load(model).placeholder(placeholders[0]).error(placeholders[1]).fallback(placeholders[2]).transform(new GlideRoundTransform(radiusDp)).into(view);
        }catch (Exception e){e.printStackTrace();}
    }
    ...
    public static void onTrimMemory(@NonNull Activity activity, int level) {
        GlideApp.with(activity).onTrimMemory(level);
    }
